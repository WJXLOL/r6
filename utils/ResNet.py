...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(512, 256, 5, stride=2, padding=1),  # [batch, 24, 8, 8]
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.ConvTranspose2d(256, 128, 3, stride=2, padding=1),  # [batch, 12, 16, 16]
            nn.ReLU(),
            nn.Dropout(0.3),
...
class ResNet_modify(nn.Module):
    ...
    def __init__(self, block, num_blocks1, num_Blocks2=[3, 3, 3], num_classes=100, nc=3, nf=64):
        super(ResNet_modify, self).__init__()
        self.in_planes = nf

        self.conv1 = nn.Conv2d(3, self.in_planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.in_planes)
        self.layer1 = self._make_layer1(block, 1 * nf, num_blocks1[0], stride=1)
        self.layer2 = self._make_layer1(block, 2 * nf, num_blocks1[1], stride=2)
        self.layer3 = self._make_layer1(block, 4 * nf, num_blocks1[2], stride=2)
        ...
        self.layer4 = self._make_layer2(BasicBlockDec, 4 * nf, num_Blocks2[2], stride=2)
        self.layer5 = self._make_layer2(BasicBlockDec, 2 * nf, num_Blocks2[1], stride=2)
        self.layer6 = self._make_layer2(BasicBlockDec, 1 * nf, num_Blocks2[0], stride=1)
    ...
    def forward(self, x, train=False):
        ...
        u = self.layer5(u)
        u = self.layer6(u)
        u = torch.sigmoid(self.bn1(u))
        u = self.conv2(u)
        u = u.view(x.size(0), 3, 32, 32)
...
class Dec(nn.Module):
    ...
    def __init__(self, num_Blocks=[3, 3, 3], z_dim=32, nc=3, nf=64):
        ...
        self.in_planes = nf
        self.bn1 = nn.BatchNorm2d(self.in_planes)
        self.layer3 = self._make_layer(BasicBlockDec, 4 * nf, num_Blocks[2], stride=2)
        self.layer2 = self._make_layer(BasicBlockDec, 2 * nf, num_Blocks[1], stride=2)
        self.layer1 = self._make_layer(BasicBlockDec, 1 * nf, num_Blocks[0], stride=1)
        self.conv1 = ResizeConv2d(nf, nc, kernel_size=3, scale_factor=1)
